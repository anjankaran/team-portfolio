  ...
  return <motion.div className="gh-page" initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
    <div className="gh-wrap">
      ...
      <div className="gh-grid"><div className="gh-files-column"><div className="gh-branch"><button>⌘ &nbsp;{repo?.default_branch ?? "main"} <ChevronRight size={13}/></button><span>{files.length} files</span></div>
        ...
        <section className="gh-reader"><div className="gh-card gh-readme"><div className="gh-readme-title"><span><Book size={15}/> {selected}</span><a href={`${link}/blob/${repo?.default_branch ?? "main"}/${selected}`} target="_blank" rel="noreferrer">View on GitHub ↗</a></div>{loading ? <p className="gh-state">Loading README…</p> : readmeError ? <article className="gh-markdown"><h1>{p.name}</h1><p>{p.headline}</p><blockquote><p>{readmeError} Showing this project summary instead. <a href={link} target="_blank" rel="noreferrer">Open repository on GitHub ↗</a></p></blockquote></article> : selected.toLowerCase().endsWith(".md") ? <MarkdownReadme source={content} /> : <pre>{content}</pre>}</div></section>
