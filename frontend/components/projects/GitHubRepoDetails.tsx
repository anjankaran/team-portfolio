...
import { ArrowLeft, ArrowUpRight, Book, GitFork, Star, ChevronRight } from "lucide-react";
...
export function GitHubRepoDetails({ p }: { p: ProjectCase }) {
  ...
  const repoName = repoPath.split("/")[1] ?? p.name;
  ...
  return <motion.div className="gh-page" initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
    <div className="gh-wrap">
      <header className="gh-header"><div><h1><span>STACKLOOP</span> / {repoName} <small>Public</small></h1><p>{repo?.description ?? `${p.name} project repository`}</p></div><div className="gh-header-actions"><button className="gh-view" onClick={() => setOpenId(null)}><ArrowLeft size={14}/> Projects</button><a className="gh-view" href={link} target="_blank" rel="noreferrer"><GithubMark/> View on GitHub <ArrowUpRight size={13}/></a></div></header>
      ...
      <div className="gh-grid"><div className="gh-files-column"><div className="gh-branch"><button>⌘ &nbsp;{repo?.default_branch ?? "main"} <ChevronRight size={13}/></button><span>{files.length} files</span></div>
        ...
        <section className="gh-reader"><div className="gh-card gh-readme"><div className="gh-readme-title"><span><Book size={15}/> {selected}</span><a href={`${link}/blob/${repo?.default_branch ?? "main"}/${selected}`} target="_blank" rel="noreferrer">View on GitHub ↗</a></div>{loading || error ? <p className="gh-state">{loading ? "Loading README…" : error}</p> : <pre>{content}</pre>}</div></section>
